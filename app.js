...
const pageNames = {dashboard:'Dashboard',crafts:'Crafts',prices:'Prix HDV',workshop:'Atelier & Courses',opportunities:'Opportunités & Conseiller',priorities:'Prix prioritaires',history:'Historique',backup:'Sauvegarde'};
...
async function openRecipe(id, name) {
  try {
    ...
    const flattenRecipeNodes = (nodes, out=[]) => {
      for (const node of (nodes || [])) { out.push(node); flattenRecipeNodes(node.children, out); }
      return out;
    };
    const editableNodes = flattenRecipeNodes(recipe.editable_tree || []);
    const missing = editableNodes.filter(x => x.buy == null);
    const renderEditableNode = node => {
      const ingredientName = node.name || '#' + node.ingredient_id;
      const bestLot = unitPrice(node.p1, node.p10, node.p100);
      const directCost = node.best == null ? null : node.best * node.total_quantity;
    ...
    $('#recipeBody').innerHTML = editableNodes.length ? `<div class="recursive-price-help"><b>Recette complète modifiable</b><small>Les lignes indentées sont les composants des composants. Tous les prix x1, x10 et x100 peuvent être enregistrés en une seule fois.</small></div><div class="recipe-price-table recursive-price-table">
      <div class="recipe-price-row header"><span>Composant / quantité</span><span>Prix x1</span><span>Prix x10</span><span>Prix x100</span><span>Meilleur PU</span><span>Coût</span><span>Navigation</span></div>${ingredientRows}</div>` : '<div class="empty">Aucune recette trouvée.</div>';
    if (missing.length) {
      $('#recipeBody').insertAdjacentHTML('afterbegin', `<div class="missing-price-banner"><div><b>${missing.length} prix HDV manquant${missing.length > 1 ? 's' : ''}</b><small>Ils peuvent être saisis directement, y compris dans les sous-recettes.</small></div><button id="openFirstMissing" class="secondary">Ouvrir le premier dans Prix HDV</button></div>`);
...
function bindRecipePriceTable() {
  const rows = $$('#recipeSummary [data-item-id], #recipeBody .recipe-price-row[data-item-id]');
  rows.forEach(row => {
    row.querySelectorAll('.money-field').forEach((input, index, fields) => {
      bindMoneyField(input);
      input.addEventListener('input', () => {
        const cls = input.classList.contains('rp1') ? '.rp1' : input.classList.contains('rp10') ? '.rp10' : '.rp100';
        rows.filter(other => other !== row && other.dataset.itemId === row.dataset.itemId).forEach(other => {
          const twin = other.querySelector(cls); if (twin && twin.value !== input.value) twin.value = input.value;
        });
      });
  ...
  saveAll.onclick = async () => {
    ...
    try {
      ...
      rows.forEach(row => uniqueItems.set(+row.dataset.itemId, {
...
// Migration automatique de l'ancienne Liste de courses vers l'Atelier.
try {
  const legacyShopping=JSON.parse(localStorage.getItem('dcm_shopping_v3')||'[]');
  for(const item of legacyShopping){
    const itemId=Number(item.item_id ?? item.id);
    const quantity=Math.max(1,Number(item.quantity)||1);
    if(!itemId)continue;
    const found=workshopSelection.find(x=>Number(x.item_id)===itemId);
    if(found)found.quantity=Math.max(1,Number(found.quantity)||1)+quantity;
    else workshopSelection.push({item_id:itemId,name:item.name||`Objet ${itemId}`,quantity});
  }
  if(legacyShopping.length)localStorage.removeItem('dcm_shopping_v3');
}catch(_error){}
let selectedWorkshopCraft=null, selectedInventoryItem=null;
let workshopPlanTimer=null;
function scheduleWorkshopPlan(){
  clearTimeout(workshopPlanTimer);
  if(!workshopLoaded)return;
  workshopPlanTimer=setTimeout(calculateWorkshopPlan,120);
}
const saveWorkshopSelection=(recalculate=true)=>{
  localStorage.setItem('dcm_workshop_selection',JSON.stringify(workshopSelection));
  renderWorkshopSelection();
  if(recalculate)scheduleWorkshopPlan();
};
function addToWorkshop(item,quantity=1){
  const itemId=Number(item.item_id ?? item.id);
  if(!itemId)return false;
  const qty=Math.max(1,Number(quantity)||1);
  const found=workshopSelection.find(x=>Number(x.item_id)===itemId);
  if(found)found.quantity=Math.max(1,Number(found.quantity)||1)+qty;
  else workshopSelection.push({item_id:itemId,name:item.name||`Objet ${itemId}`,quantity:qty});
...
$('#workshopAddCraft')?.addEventListener('click',()=>{if(!selectedWorkshopCraft)return;addToWorkshop(selectedWorkshopCraft,+$('#workshopCraftQty').value||1);selectedWorkshopCraft=null;showSelected('craft',null);$('#workshopCraftSearch').value='';$('#workshopCraftMatches').innerHTML='';});
$('#addModalShopping')?.addEventListener('click',()=>{if(!currentRecipe)return;const qty=Math.max(1,+$('#modalQty').value||1);if(addToWorkshop(currentRecipe,qty)){ $('#recipeModal').classList.add('hidden'); switchPage('workshop'); }});
$('#workshopClear')?.addEventListener('click',()=>{workshopSelection=[];saveWorkshopSelection(false);renderWorkshopPlan(null);$('#workshopState').textContent='Liste vidée.';});
async function loadInventory(){
  ...
  $$('#inventoryList .inventory-line').forEach(row=>{
    const save=async quantity=>{await api('/api/inventory',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({item_id:+row.dataset.id,quantity:Math.max(0,quantity)})});await loadInventory();scheduleWorkshopPlan();};
...
async function calculateWorkshopPlan(){
  if(!workshopSelection.length){$('#workshopState').textContent='Ajoute au moins un craft.';renderWorkshopPlan(null);return;}
  $('#workshopState').textContent='Calcul de la liste de courses…';
  try{
    const data=await api('/api/workshop-plan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({selections:workshopSelection})});
    renderWorkshopPlan(data);
  }catch(error){$('#workshopState').textContent='Calcul impossible : '+error.message;}
}
$('#workshopCalculate')?.addEventListener('click',calculateWorkshopPlan);
...
async function loadWorkshopOnce(){if(workshopLoaded)return;workshopLoaded=true;renderWorkshopSelection();renderWorkshopPlan(null);await loadInventory();if(workshopSelection.length)await calculateWorkshopPlan();}
